...
# 1) Жадный подход: Алгоритм всегда выбирает ближайший город, который еще не был посещен.
#    Это может не всегда приводить к глобально оптимальному решению, но часто дает хорошее приближение.
# 2) Матрица расстояний M: Используется для хранения расстояний между городами. 
#    По мере выполнения алгоритма, элементы этой матрицы обновляются, чтобы исключить уже посещенные города.
# 3) Конечный маршрут: В конце выполнения алгоритма way будет содержать порядок посещения всех городов, начиная с города ib.
way.append(ib)
